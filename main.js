...
function beginTest() {
    ...
    $("div#status").html("Test ready - Just start tapping");
    $("div#Result").html("\
		Tap Speed: 0 taps / 0 seconds ー\
        Stream Speed: 0 bpm ー\
        Unstable Rate: 0\
    ");
...
function update(click) {
	...
	if (timediffs.length < 2) {
		$("div#Result").html("\
		Tap Speed: " + (clickTimes.length.toString() + " taps / " + streamtime.toFixed(3)) + " seconds ー\
		Stream Speed: " + (Math.round((((clickTimes.length) / (Date.now() - beginTime) * 60000)/4) * 100) / 100).toFixed(2) + " bpm ー\
		Unstable Rate: n/a\
	");
	} else {
	    $("div#Result").html("\
